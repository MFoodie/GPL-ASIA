    ...
    def set_prompt_graph(self, prompt_graph=None):
        if prompt_graph is None:
            ...
            with tc.no_grad():
                ...
                prompt_graph = z + tc.randn_like(z) * 0.1
            self.train()
...
def evaluate_with_metrics(perturbed_x, perturbed_edge_index, mask=None):
    ...
    y_pred_pos = F.softmax(output[(mask & data.test_mask.to(device)) if mask is not None else data.test_mask.to(device)], dim=1).cpu().detach().numpy().max(axis=1)
    y_pred_neg = F.softmax(output[~(mask & data.test_mask.to(device)) if mask is not None else ~data.test_mask.to(device)], dim=1).cpu().detach().numpy().max(axis=1)
    ...
    print(f"平均倒数排名 (MRR): {mrr_score['mrr']:.4f}")
...
def evaluate_attack_metrics(perturbed_x, perturbed_edge_index, mask=None, results=None):
    ...
    original_confidence = calculate_confidence(model(data.x, data.edge_index), (mask & data.test_mask.to(device)) if mask is not None else data.test_mask.to(device))
    perturbed_confidence = calculate_confidence(model(perturbed_x, perturbed_edge_index), (mask & data.test_mask.to(device)) if mask is not None else data.test_mask.to(device))
