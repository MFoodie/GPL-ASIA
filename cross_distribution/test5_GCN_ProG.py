...
class GCN(nn.Module):
    ...
    def set_prompt_graph(self, prompt_graph=None):
        if prompt_graph is None:
            # ProG initialization: Use input features and adjacency-based weighting
            self.eval()
            with tc.no_grad():
                # Use data.x (shape: [2708, 1433]) as base
                adj = tc.zeros((data.num_nodes, data.num_nodes), device=device)
                adj[data.edge_index[0], data.edge_index[1]] = 1
                degree_matrix = tc.diag(tc.pow(adj.sum(dim=1), -0.5))
                normalized_adj = degree_matrix @ adj @ degree_matrix
                prompt_graph = data.x * normalized_adj.mean(dim=1).unsqueeze(1)
        ...
        self.prompt_optimizer = optim.Adam([self.prompt_graph], lr=0.01, weight_decay=1e-4)  # Regularization
...
def train_with_contrastive_loss(mask=None):
    ...
    perturbed_x1, perturbed_edge_index1 = enhanced_stealthy_adversarial_attack(data, model, perturbation_ratio=0.3, mask=mask)
    perturbed_x2, perturbed_edge_index2 = enhanced_stealthy_adversarial_attack(data, model, perturbation_ratio=0.3, mask=mask)
...
def enhanced_stealthy_adversarial_attack(data, model, epsilon=0.2, perturbation_ratio=0.3, task_type='node_classification', mask=None):
    model.eval()
...
def enhanced_perturb_node_features(x, model, perturbation_ratio=0.3, epsilon=0.2, task_type='node_classification', mask=None):
...
def evaluate_with_metrics(perturbed_x, perturbed_edge_index, mask=None):
    def evaluate_attack_metrics(perturbed_x, perturbed_edge_index, mask=None, results=None):
        ...
        y_pred_probs = F.softmax(
            model(perturbed_x, perturbed_edge_index)[(mask & data.test_mask.to(device)) if mask is not None else data.test_mask.to(device)],
