...
for name, mask in sub_distributions.items():
    ...
    if num_nodes == 0:
        ...
        continue  # Skip empty sub-distributions but continue processing others
results = {}
...
class GAT(nn.Module):
    def __init__(self, nfeat, nclass, nhid, dropout, nheads):
        ...
        self.conv2 = GATConv(nhid * nheads, nclass, heads=1, dropout=dropout)
    ...
    def forward(self, x, edge_index):
        if self.prompt_graph is not None:
            x = x + self.prompt_graph  # Add prompt to input features
    ...
    def set_prompt_graph(self, prompt_graph=None):
        if prompt_graph is None:
            # ProG initialization: Use input features and degree-based weighting
            self.eval()
            with tc.no_grad():
                # Use data.x (shape: [2708, 1433]) as base
                degrees = degree(data.edge_index[0], num_nodes=data.num_nodes).float().to(device)
                normalized_degrees = degrees / degrees.max()
                # Initialize prompt_graph with the same feature dimension as data.x (1433)
                prompt_graph = data.x * normalized_degrees.unsqueeze(1)
                prompt_graph += tc.randn_like(data.x) * 0.5  # Small noise for diversity
            self.train()
        self.prompt_graph = nn.Parameter(prompt_graph.to(device), requires_grad=True)
        self.prompt_optimizer = optim.Adam([self.prompt_graph], lr=0.01, weight_decay=1e-4)  # Add weight decay
...
dropout = 0.6
nheads = 8
model = GAT(nfeat, nclass, nhid, dropout, nheads).to(device)
...
def train_with_contrastive_loss(mask=None):
    ...
    perturbed_x1, perturbed_edge_index1 = stealthy_adversarial_attack(data, model, perturbation_ratio=0.2, mask=mask)
    perturbed_x2, perturbed_edge_index2 = stealthy_adversarial_attack(data, model, perturbation_ratio=0.2, mask=mask)
    ...
    loss = contrastive_loss(output1[mask] if mask is not None else output1, output2[mask] if mask is not None else output2)
    loss.backward()
    optimizer.step()
    if model.prompt_graph is not None:
...
def train(mask=None):
    ...
    loss = criterion(output[mask & data.train_mask.to(device)] if mask is not None else output[data.train_mask.to(device)], data.y.to(device)[mask & data.train_mask.to(device)] if mask is not None else data.y.to(device)[data.train_mask.to(device)])
    loss.backward()
    optimizer.step()
    if model.prompt_graph is not None:
        model.prompt_optimizer.step()
...
def stealthy_adversarial_attack(data, model, epsilon=0.15, perturbation_ratio=0.4, task_type='node_classification', mask=None):
    model.eval()
    num_nodes = data.x.size(0) if mask is None else mask.sum().item()
    original_acc = test(mask)
    perturbation_ratio = min(perturbation_ratio, 0.5 * (1 - original_acc))

    target_node = select_low_centrality_node(data, task_type, mask)
    perturbed_edge_index = add_trigger_nodes(data, target_node, num_trigger_nodes=5 if task_type == 'node_classification' else 3, mask=mask)
    perturbed_edge_index = add_trigger_induced_subgraph(data, perturbation_ratio, task_type, mask)
    perturbed_edge_index = structure_perturbation(data, perturbed_edge_index, perturbation_ratio, mask)
...
# 选择低中心性节点
def select_low_centrality_node(data, task_type='node_classification', mask=None):
    ...
    if task_type == 'node_classification':
        ...
        class_counts = tc.bincount(data.y.to(device)[data.train_mask.to(device) & (mask if mask is not None else tc.ones_like(data.train_mask.to(device), dtype=bool))])
...
# 触发节点攻击
def add_trigger_nodes(data, target_node, num_trigger_nodes=5, mask=None):
...
# 触发诱导子图攻击
def add_trigger_induced_subgraph(data, perturbation_ratio=0.2, task_type='node_classification', mask=None):
    num_nodes = data.x.size(0)
    ...
    degrees = degree(data.edge_index[0], num_nodes=num_nodes)
    test_nodes = tc.where(data.test_mask.to(device) & (mask if mask is not None else tc.ones_like(data.test_mask.to(device), dtype=bool).to(device)))[0]
    if len(test_nodes) > 0:
        test_degrees = degrees[test_nodes]
...
# 结构扰动
def structure_perturbation(data, edge_index, perturbation_ratio=0.2, mask=None):
    ...
    new_edges = set()
    test_nodes = tc.where(data.test_mask.to(device) & (mask if mask is not None else tc.ones_like(data.test_mask.to(device), dtype=bool).to(device)))[0].cpu().numpy()
...
# 特征扰动
def perturb_node_features(x, perturbation_ratio=0.2, epsilon=0.15, task_type='node_classification', mask=None):
    x = x.clone().detach().requires_grad_(True).to(device)
    output = model(x, data.edge_index)
    loss = F.cross_entropy(output[data.train_mask.to(device) & (mask if mask is not None else tc.ones_like(data.train_mask.to(device), dtype=bool))], data.y.to(device)[data.train_mask.to(device) & (mask if mask is not None else tc.ones_like(data.train_mask.to(device), dtype=bool))])
    model.zero_grad()
    loss.backward()
    grad = x.grad
...
def test(mask=None):
    ...
    correct = (pred[(mask & data.test_mask.to(device)) if mask is not None else data.test_mask.to(device)] == data.y.to(device)[(mask & data.test_mask.to(device)) if mask is not None else data.test_mask.to(device)]).sum()
...
def evaluate_with_metrics(perturbed_x, perturbed_edge_index, mask=None):
    ...
    y_pred_pos = F.softmax(output[(mask & data.test_mask.to(device)) if mask is not None else data.test_mask.to(device)], dim=1).cpu().detach().numpy()
    y_pred_neg = F.softmax(output[~(mask & data.test_mask.to(device)) if mask is not None else ~data.test_mask.to(device)], dim=1).cpu().detach().numpy()
    ...
    if len(y_pred_pos_max) != len(y_pred_neg_max):
        ...
        min_samples = min(len(y_pred_pos_max), len(y_pred_neg_max))
        y_pred_pos_max = np.random.choice(y_pred_pos_max, min_samples, replace=False)
        y_pred_neg_max = np.random.choice(y_pred_neg_max, min_samples, replace=False)
...
def evaluate_attack_metrics(perturbed_x, perturbed_edge_index, mask=None, results=None):
    original_pred = model(data.x + (model.prompt_graph if model.prompt_graph is not None else 0), data.edge_index).argmax(dim=1)
    ...
    original_confidence = calculate_confidence(model(data.x + (model.prompt_graph if model.prompt_graph is not None else 0), data.edge_index), (mask & data.test_mask.to(device)) if mask is not None else data.test_mask.to(device))
    perturbed_confidence = calculate_confidence(model(perturbed_x + (model.prompt_graph if model.prompt_graph is not None else 0), perturbed_edge_index), (mask & data.test_mask.to(device)) if mask is not None else data.test_mask.to(device))
    ...
    y_true = data.y.to(device)[(mask & data.test_mask.to(device)) if mask is not None else data.test_mask.to(device)].cpu().numpy()
    y_pred = perturbed_pred[(mask & data.test_mask.to(device)) if mask is not None else data.test_mask.to(device)].cpu().numpy()
    ...
    # 不再绘制混淆矩阵，仅计算 specificity 所需的 cm
    cm = confusion_matrix(y_true, y_pred)

    y_pred_probs = F.softmax(model(perturbed_x, perturbed_edge_index)[(mask & data.test_mask.to(device)) if mask is not None else data.test_mask.to(device)], dim=1).detach().cpu().numpy()
...
def evaluate_resource_usage(perturbed_x, perturbed_edge_index, mask=None):
    ...
    start_time = time.time()
    with tc.no_grad():
        perturbed_output = model(perturbed_x + (model.prompt_graph if model.prompt_graph is not None else 0), perturbed_edge_index)
...
def profile_computation(perturbed_x, perturbed_edge_index, mask=None):
    ...
    prof.export_chrome_trace("gat_performance_trace.json")
    print("性能分析跟踪已保存到 gat_performance_trace.json")
...
if __name__ == "__main__":
    ...
    for sub_dist_name, mask in sub_distributions.items():
        ...
        try:
            ...
            # 评估攻击指标，传递 results
            asr_val = evaluate_attack_metrics(perturbed_x, perturbed_edge_index, mask, results)
    ...
    if len(results) > 0:
        ...
        labels, values = [], []
        for k in order:
            if k in results and 'asr' in results[k]:
                labels.append(k)
