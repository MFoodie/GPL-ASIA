...
class GAT(nn.Module):
    ...
    def forward(self, x, edge_index):
        if self.prompt_graph is not None:
            x = x + self.prompt_graph  # ProG: Add prompt graph to input features
...
def evaluate_attack_metrics(model, data, perturbed_x, perturbed_edge_index, dataset_name):
    try:
        ...
        plt.figure()
        sns.heatmap(confusion_matrix(y_true, y_pred), annot=True, fmt='d', cmap='cividis_r', xticklabels=class_names,
...
if __name__ == "__main__":
    for dataset_name in datasets:
        ...
        nclass = tc.unique(data.y.to(device)).size(0)  # 修正：从 data.y.to(device) 计算类别数
        nhid = 8
        dropout = 0.6
        nheads = 8
