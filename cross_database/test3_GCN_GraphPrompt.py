...
def evaluate_attack_metrics(model, data, perturbed_x, perturbed_edge_index, dataset_name):
    try:
        ...
        plt.figure()
        sns.heatmap(confusion_matrix(y_true, y_pred), annot=True, fmt='d', cmap='viridis_r', xticklabels=class_names,
...
if __name__ == "__main__":
    for dataset_name in datasets:
        ...
        nhid = 16  # 调整隐藏层维度以匹配GAT的容量
        dropout = 0.6
        model = GCN(nfeat, nclass, nhid, dropout).to(device)
        optimizer = optim.Adam(model.parameters(), lr=0.1, weight_decay=5e-4)  # 调整学习率
