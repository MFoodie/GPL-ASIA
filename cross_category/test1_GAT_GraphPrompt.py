...
class GAT(nn.Module):
    ...
    def set_prompt_graph(self, prompt_graph=None):
        ...
        self.prompt_graph = nn.Parameter(prompt_graph, requires_grad=True)
...
def evaluate_with_metrics(perturbed_x, perturbed_edge_index):
    ...
    y_pred_pos = F.softmax(output[data.test_mask.to(device)], dim=1).cpu().detach().numpy()
    y_pred_neg = F.softmax(output[~data.test_mask.to(device)], dim=1).cpu().detach().numpy()
...
def evaluate_attack_metrics_per_category(perturbed_x, perturbed_edge_index, category_mask, category_id):
    ...
    print(f'Category {category_id} Attack Success Rate (ASR): {asr:.4f} - {asr_level}')

    feature_perturbation_ratio = calculate_feature_perturbation_ratio(data.x, perturbed_x)
    print(f'Category {category_id} Perturbation Ratio: {feature_perturbation_ratio:.4f}')
    ...
    print(f'Category {category_id} Original Confidence: {original_confidence:.4f}')
    print(f'Category {category_id} Perturbed Confidence: {perturbed_confidence:.4f}')
    print(f'Category {category_id} Confidence Drop: {confidence_drop:.4f}')

    y_true = (data.y.to(device)[category_mask] == category_id).cpu().numpy()  # 单类别内的二分类标签
    ...
    print(f'Category {category_id} Precision: {precision:.4f}')
    print(f'Category {category_id} Recall: {recall:.4f}')
    print(f'Category {category_id} F1-Score: {f1:.4f}')

    # 跳过 ROC AUC 计算，改为报告其他指标
    print(f'Category {category_id} ROC-AUC: N/A (Single class in y_true, not defined)')
    ...
    print(f'Category {category_id} Specificity: {specificity:.4f}')
    return asr
...
def cross_category_test():
    ...
    # 存储最后一次攻击的结果用于性能分析
    last_perturbed_x = None
    last_perturbed_edge_index = None
    ...
    for category in range(num_classes):
        ...
        # 评估原始和攻击后的准确率
        original_acc = test()
        print(f'Category {category} Original Accuracy (Before Attack): {original_acc:.4f}')
        perturbed_acc = test_after_attack(perturbed_x, perturbed_edge_index)
        print(f'Category {category} Perturbed Accuracy (After Attack): {perturbed_acc:.4f}')

        # 评估攻击指标（针对特定类别）
        asr = evaluate_attack_metrics_per_category(perturbed_x, perturbed_edge_index, category_mask, category)
        asr_results[category] = asr

        # 评估其他指标
        hits_at_k, mrr_score = evaluate_with_metrics(perturbed_x, perturbed_edge_index)
        ...
        # 保存最后一次攻击的结果
        last_perturbed_x = perturbed_x
        last_perturbed_edge_index = perturbed_edge_index
    ...
    for category, results in category_results.items():
        asr = asr_results.get(category, 0)
        if asr > 0:
            total_asr += asr
            valid_categories += 1
            
        print(f"类别 {category}:")
        print(f"  原始准确率: {results['original_acc']:.4f}")
        print(f"  攻击后准确率: {results['perturbed_acc']:.4f}")
        print(f"  攻击成功率 (ASR): {asr:.4f}")
        print(f"  Hits@50: {results['hits_at_k']:.4f}")
        print(f"  MRR: {results['mrr']:.4f}")
    
    # 总体成功率评估
    if valid_categories > 0:
        avg_asr = total_asr / valid_categories
        print(f"\n=== 总体攻击成功率评估 ===")
        print(f"平均ASR: {avg_asr:.4f}")
        
        if avg_asr >= 0.8:
            overall_level = "优秀 (Excellent)"
        elif avg_asr >= 0.6:
            overall_level = "良好 (Good)"
        elif avg_asr >= 0.4:
            overall_level = "一般 (Fair)"
        elif avg_asr >= 0.2:
            overall_level = "较差 (Poor)"
        else:
            overall_level = "很差 (Very Poor)"
            
        print(f"总体攻击效果: {overall_level}")
        print(f"成功攻击类别数: {valid_categories}/{num_classes}")
    else:
        print("\n=== 总体攻击成功率评估 ===")
        print("没有有效的攻击结果")
    ...
    plt.title('ASR Comparison Across Categories (Cora Dataset)')
    plt.ylim(0, 1)
    ...
    # 仅在最后执行一次资源使用评估和性能分析
    if last_perturbed_x is not None and last_perturbed_edge_index is not None:
        evaluate_resource_usage(last_perturbed_x, last_perturbed_edge_index)
        if tc.cuda.is_available():
            profile_computation(last_perturbed_x, last_perturbed_edge_index)
