...
def train_with_contrastive_loss():
    ...
    # 确保设备一致
    perturbed_x1, perturbed_edge_index1 = perturbed_x1.to(device), perturbed_edge_index1.to(device)
    perturbed_x2, perturbed_edge_index2 = perturbed_x2.to(device), perturbed_edge_index2.to(device)
...
# 增强型对抗攻击
def enhanced_stealthy_adversarial_attack(data, model, epsilon=0.2, perturbation_ratio=0.3, task_type='node_classification'):
    """
    ...
    target_node = select_influential_node(data, model, task_type)
    perturbed_edge_index = add_trigger_nodes(data, target_node, num_trigger_nodes=8)
    perturbed_edge_index = add_trigger_induced_subgraph(data, perturbation_ratio * 1.2, task_type)
    perturbed_edge_index = enhanced_structure_perturbation(data, perturbed_edge_index, perturbation_ratio)
    ...
    while perturbed_acc > original_acc * 0.8 and attempt < max_attempts:
        perturbation_ratio = min(perturbation_ratio * 1.2, 0.7)
        epsilon = min(epsilon * 1.1, 0.4)
    ...
    # 保证返回张量在正确设备
    return perturbed_x.to(device), perturbed_edge_index.to(device)
...
def crossba_perturb_node_features(x, model, perturbation_ratio, epsilon, target_embedding, 
    ...
    for idx in important_features:
        col = int(idx.item())
        perturbed_x[:, col] += adaptive_epsilon[col] * grad[:, col].sign()
    ...
    for idx in noise_features:
        col = int(idx.item())
        noise = tc.randn_like(perturbed_x[:, col]) * adaptive_epsilon[col] * 0.5
    ...
    # 保证设备一致
    return perturbed_x.to(x.device)

def select_influential_node(data, model, task_type='node_classification'):
    ...
    if len(test_nodes) > 0:
        test_uncertainty = uncertainty[test_nodes]
...
def add_trigger_nodes(data, target_node, num_trigger_nodes=8):
...
def add_trigger_induced_subgraph(data, perturbation_ratio=0.3, task_type='node_classification'):
    num_nodes = data.x.size(0)
...
def enhanced_structure_perturbation(data, edge_index, perturbation_ratio=0.3):
...
def enhanced_perturb_node_features(x, model, perturbation_ratio=0.3, epsilon=0.2, task_type='node_classification'):
    ...
    for idx in important_features:
        col = int(idx.item())
        noise = adaptive_epsilon[col] * grad[:, col].sign() + tc.randn_like(x[:, col]) * 0.5
        perturbed_x[:, col] += noise
    perturbed_x = tc.clamp(perturbed_x, min=x.min(), max=x.max())
...
def perform_attack_and_finetune():
    perturbed_x, perturbed_edge_index = enhanced_stealthy_adversarial_attack(data, model)
...
def test_after_attack(perturbed_x, perturbed_edge_index):
    model.eval()
    output = model(perturbed_x.to(device), perturbed_edge_index.to(device))
    pred = output.argmax(dim=1)
    correct = (pred[data.test_mask.to(device)] == data.y.to(device)[data.test_mask.to(device)]).sum()
    acc = int(correct) / int(data.test_mask.to(device).sum())
    return acc
...
def evaluate_attack_metrics_per_category(perturbed_x, perturbed_edge_index, category_mask, category_id):
    ...
    perturbed_pred = model(perturbed_x.to(device), perturbed_edge_index.to(device)).argmax(dim=1)
    asr = calculate_asr(original_pred, perturbed_pred, category_mask, data.y.to(device))
    print(f'类别 {category_id} 攻击成功率 (ASR): {asr:.4f}')
    feature_perturbation_ratio = calculate_feature_perturbation_ratio(data.x, perturbed_x.to(device))
    ...
    perturbed_confidence = calculate_confidence(model(perturbed_x.to(device), perturbed_edge_index.to(device)), category_mask)
    ...
    print(f'类别 {category_id} F1 分数: {f1:.4f}')
    print(f'类别 {category_id} ROC-AUC: N/A (单类标签无法计算)')
    cm = confusion_matrix(y_true, y_pred == category_id)
    class_names = ['非类别 ' + str(category_id), '类别 ' + str(category_id)]
    plt.figure(figsize=(8, 6))
    sns.heatmap(cm, annot=True, fmt='d', cmap='inferno_r', xticklabels=class_names, yticklabels=class_names)
    plt.xlabel('预测值')
    plt.ylabel('真实值')
    plt.title(f'Cora - 混淆矩阵 (类别 {category_id})')
    plt.show()
    ...
    print(f'类别 {category_id} 特异性: {specificity:.4f}')
    return asr
...
def evaluate_resource_usage(perturbed_x, perturbed_edge_index):
    ...
    start_time = time.time()
    with tc.no_grad():
        perturbed_output = model(perturbed_x.to(device), perturbed_edge_index.to(device))
...
def profile_computation(perturbed_x, perturbed_edge_index):
    ...
    with tc.profiler.profile(
        ...
        with tc.no_grad():
            ...
            model(perturbed_x.to(device), perturbed_edge_index.to(device))
...
if __name__ == "__main__":
    model.set_prompt_graph()  # 初始化提示图
    for epoch in range(100):
        loss = train_with_contrastive_loss()
        if epoch % 10 == 0:
            print(f'预训练轮次: {epoch:03d}, 损失: {loss:.4f}')
    ...
    for i, v in enumerate([asr_results.get(cat, 0) for cat in range(num_classes)]):
        # 避免越界显示
        y = min(0.98, max(0.0, float(v)))
        plt.text(i, y, f'{v:.2f}' if v > 0 else 'N/A', ha='center', va='bottom')
