...
# ProG模型定义
class GAE(nn.Module):
    def __init__(self, nfeat, nclass, nhid, dropout, n_layers=3):
        super(GAE, self).__init__()
        self.convs = nn.ModuleList([GCNConv(nfeat if i == 0 else nhid, nhid) for i in range(n_layers)])
        self.fc = nn.Linear(nhid, nclass)  # 分类层
    ...
    def encode(self, x, edge_index):
        for i, conv in enumerate(self.convs):
            ...
            if i < self.n_layers - 1:
                x = F.normalize(x, p=2, dim=1)  # 渐进式归一化
        return x

    def decode(self, z, edge_index):
        # 简单的内积解码器，重构邻接矩阵
        adj_rec = tc.sigmoid(tc.matmul(z, z.t()))
        return adj_rec
    ...
    def forward(self, x, edge_index, batch):
        ...
        # 分类输出
        return F.log_softmax(self.fc(z), dim=1)

# 模型参数 - 针对GAE优化
nfeat = dataset.num_features
nhid = 32  # 增加隐藏层维度，提高模型容量
nclass = dataset.num_classes
dropout = 0.2  # 适度dropout，提高泛化能力
n_layers = 3  # ProG 使用多层渐进式更新

model = GAE(nfeat, nclass, nhid, dropout, n_layers).to(device)
...
def enhanced_perturb_node_features(x, attack_node_ids, attack_strategy='adaptive', noise_scale=0.1):
    ...
    if attack_strategy == 'adaptive':
        ...
        for node_id in attack_node_ids:
            ...
            noise = tc.randn_like(x[node_id]) * noise_scale * importance_weights
            perturbed_x[node_id] = x[node_id] + noise
            
    elif attack_strategy == 'degree_based':
        # 基于度的扰动策略（需要传入edge_index参数）
        # 这里简化为随机扰动，因为degree信息需要从外部传入
        for node_id in attack_node_ids:
            noise = tc.randn_like(x[node_id]) * noise_scale
            perturbed_x[node_id] = x[node_id] + noise
...
def adaptive_attack_strategy(model, data, attack_round=0):
    ...
    # 动态调整攻击参数
    if confidence > 0.8:
        noise_scale = 0.15 + attack_round * 0.02  # 高置信度时增加扰动
        attack_ratio = 0.3 + attack_round * 0.05
    elif confidence > 0.6:
        noise_scale = 0.1 + attack_round * 0.01
        attack_ratio = 0.2 + attack_round * 0.03
    else:
        noise_scale = 0.05 + attack_round * 0.005
        attack_ratio = 0.1 + attack_round * 0.02
    ...
    return min(noise_scale, 0.3), min(attack_ratio, 0.5)
...
def stealthy_adversarial_attack(data, model, attack_round=0):
    ...
    # 特征扰动 - 针对GAE使用编码器聚焦策略
    attack_strategy = 'encoder_focused' if attack_round % 4 == 0 else 'adaptive'
...
def evaluate(data_loader, device):
    ...
    sns.heatmap(cm, annot=True, fmt='d', cmap='inferno_r', vmin=0, vmax=cm.max())  # 修正颜色条范围
