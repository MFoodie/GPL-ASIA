...
# ProG Model Definition
class GAT(nn.Module):
    def __init__(self, nfeat, nclass, nhid, dropout, nheads):
        super(GAT, self).__init__()
        # Graph encoder (GAT layers)
        self.conv1 = GATConv(nfeat, nhid, heads=nheads, dropout=dropout)
        self.conv2 = GATConv(nhid * nheads, nhid, heads=1, dropout=dropout)

        # Prompt components
        self.prompt = nn.Parameter(tc.randn(nhid, nhid).to(device))
    ...
    def forward(self, x, edge_index, batch):
        ...
        # Prompt application (ProG's key component)
        x = self.prompt_activation(x @ self.prompt)
...
dropout = 0.3  # 降低dropout，提高模型容量
nheads = 4  # 减少注意力头数，避免过度复杂
...
def enhanced_perturb_node_features(x, attack_node_ids, attack_strategy='adaptive', noise_scale=0.1):
    ...
    if attack_strategy == 'adaptive':
        ...
        for node_id in attack_node_ids:
            ...
            # 增加扰动强度，特别是对重要特征
            enhanced_noise_scale = noise_scale * 1.5  # 增加50%的扰动强度
            noise = tc.randn_like(x[node_id]) * enhanced_noise_scale * importance_weights
            
            # 添加方向性扰动，更容易影响注意力权重
            directional_noise = tc.sign(x[node_id]) * tc.abs(tc.randn_like(x[node_id])) * noise_scale * 0.3
            perturbed_x[node_id] = x[node_id] + noise + directional_noise
            
    elif attack_strategy == 'attention_focused':
        # 针对注意力机制的专门扰动策略
        for node_id in attack_node_ids:
            # 对特征进行更大幅度的扰动，影响注意力计算
            attention_noise = tc.randn_like(x[node_id]) * noise_scale * 2.0
            # 添加特征缩放扰动，影响注意力权重
            scale_factor = 0.8 + tc.rand(1).item() * 0.4  # 0.8-1.2的随机缩放
            perturbed_x[node_id] = x[node_id] * scale_factor + attention_noise
            
    elif attack_strategy == 'degree_based':
        # 基于度的扰动策略
        for node_id in attack_node_ids:
            noise = tc.randn_like(x[node_id]) * noise_scale * 1.2
            perturbed_x[node_id] = x[node_id] + noise
            
    else:  # random
        # 随机扰动，增加强度
        for node_id in attack_node_ids:
            noise = tc.randn_like(x[node_id]) * noise_scale * 1.3
            perturbed_x[node_id] = x[node_id] + noise
...
def adaptive_attack_strategy(model, data, attack_round=0):
    ...
    # 针对GAT的动态调整攻击参数（更激进的策略）
    if confidence > 0.8:
        noise_scale = 0.25 + attack_round * 0.03  # 增加基础扰动强度
        attack_ratio = 0.4 + attack_round * 0.06  # 增加攻击节点比例
    elif confidence > 0.6:
        noise_scale = 0.18 + attack_round * 0.02
        attack_ratio = 0.3 + attack_round * 0.04
    else:
        noise_scale = 0.12 + attack_round * 0.015
        attack_ratio = 0.2 + attack_round * 0.03
    
    # 根据不确定性调整（GAT对不确定性更敏感）
    if entropy > 1.0:  # 高不确定性
        noise_scale *= 1.4  # 增加扰动强度
        attack_ratio *= 1.2
    elif entropy < 0.5:  # 低不确定性，需要更强攻击
        noise_scale *= 1.3
        attack_ratio *= 1.15
    
    # 根据图复杂度调整
    if degree_variance > 10.0:  # 复杂图结构
        noise_scale *= 0.9  # 稍微降低，但不要太多
        attack_ratio *= 0.95
    else:  # 简单图结构，可以更激进
        noise_scale *= 1.1
        attack_ratio *= 1.05
    ...
    # 针对GAT的额外调整
    noise_scale *= 1.2  # 整体增加20%的扰动强度
    attack_ratio *= 1.1  # 整体增加10%的攻击节点比例
    
    return min(noise_scale, 0.5), min(attack_ratio, 0.7)  # 提高上限
...
def stealthy_adversarial_attack(data, model, attack_round=0):
    ...
    # 特征扰动 - 针对GAT使用注意力聚焦策略
    attack_strategy = 'attention_focused' if attack_round % 3 == 0 else 'adaptive'
