...
# 定义 GAE + 分类器（ProG 提示图加到输入特征）
class GAE_ProG(nn.Module):
    def __init__(self, nfeat, nclass, nhid, dropout):
        super().__init__()
        self.gae = GAE(GCNEncoder(nfeat, nhid, dropout))
...
model = GAE_ProG(nfeat, nclass, nhid, dropout).to(device)
...
def train_with_contrastive_loss():
    ...
    z1 = model.gae.encode(x1 + prog_graph, e1)
    z2 = model.gae.encode(x2 + prog_graph, e2)
...
def evaluate_attack_metrics():
    ...
    plt.xlabel('Predicted'); plt.ylabel('True'); plt.title('Cora - Confusion Matrix')
    plt.show()
