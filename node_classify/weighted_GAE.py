...
class WeightedGAE(GAE):
    def __init__(self, nfeat, nhid, dropout):
        encoder = GCNEncoder(nfeat, nhid, dropout)
        super(WeightedGAE, self).__init__(encoder=encoder)
    ...
    def encode(self, x, edge_index, edge_weight=None):
        ...
        x = F.elu(self.encoder.conv1(x, edge_index, edge_weight))
    ...
    def decode(self, z, edge_index, edge_weight=None):
        return tc.sigmoid((z[edge_index[0]] * z[edge_index[1]]).sum(dim=1))
...
nhid = 8
dropout = 0.6
model = WeightedGAE(nfeat, nhid, dropout)
...
def train_with_contrastive_loss():
    ...
    z1 = model.encode(perturbed_x1, perturbed_edge_index1, perturbed_weight1)
    z2 = model.encode(perturbed_x2, perturbed_edge_index2, perturbed_weight2)
...
def select_high_impact_node(data, model):
    ...
    with tc.no_grad():
        ...
        for i in range(min(100, data.num_nodes)):  # 限制计算量
            # 计算移除该节点后的损失变化
            original_z = model.encode(data.x, data.edge_index, data.edge_weight)
            original_loss = F.binary_cross_entropy(
                model.decode(original_z, data.edge_index, data.edge_weight),
                tc.ones_like(model.decode(original_z, data.edge_index, data.edge_weight))
            )
            
            # 模拟移除节点i的影响
            mask = tc.ones(data.num_nodes, dtype=tc.bool)
            mask[i] = False
            masked_x = data.x.clone()
            masked_x[i] = 0  # 将节点i的特征置零
            
            masked_z = model.encode(masked_x, data.edge_index, data.edge_weight)
            masked_loss = F.binary_cross_entropy(
                model.decode(masked_z, data.edge_index, data.edge_weight),
                tc.ones_like(model.decode(masked_z, data.edge_index, data.edge_weight))
            )
...
def generate_target_embedding(data, model, target_node):
    ...
    with tc.no_grad():
        ...
        # 获取触发器嵌入
        target_embedding = model.encode(trigger_x, trigger_edge_index, trigger_edge_weight)
...
def crossba_perturb_node_features(x, model, edge_index, edge_weight, perturbation_ratio, epsilon, 
    ...
    adj_pred = model.decode(z, edge_index, edge_weight)
    adj_true = tc.ones_like(adj_pred)
    ...
    # 策略2: CrossBA隐蔽性保持
    with tc.no_grad():
        original_emb = model.encode(x, edge_index, edge_weight)
        perturbed_emb = model.encode(perturbed_x, edge_index, edge_weight)
...
def test():
    model.eval()
    z = model.encode(data.x, data.edge_index, data.edge_weight)
    adj_pred = model.decode(z, data.edge_index, data.edge_weight)
    adj_true = tc.ones_like(adj_pred)  # 假设边存在为 1
    acc = (adj_pred > 0.5).float().eq(adj_true).float().mean().item()
    return acc
...
def test_after_attack(perturbed_x, perturbed_edge_index, perturbed_edge_weight):
    model.eval()
    z = model.encode(perturbed_x, perturbed_edge_index, perturbed_edge_weight)
    adj_pred = model.decode(z, perturbed_edge_index, perturbed_edge_weight)
    adj_true = tc.ones_like(adj_pred)
    acc = (adj_pred > 0.5).float().eq(adj_true).float().mean().item()
    return acc
...
def evaluate_attack_metrics():
    # 添加线性层将嵌入转换为分类 logits
    classifier = nn.Linear(model.nhid, dataset.num_classes).to(data.x.device)
    model.eval()
    with tc.no_grad():
        original_z = model.encode(data.x, data.edge_index, data.edge_weight)
        perturbed_z = model.encode(perturbed_x, perturbed_edge_index, perturbed_edge_weight)
        original_pred = classifier(original_z).argmax(dim=1)
        perturbed_pred = classifier(perturbed_z).argmax(dim=1)
    ...
    original_confidence = calculate_confidence(classifier(original_z), data.test_mask)
    perturbed_confidence = calculate_confidence(classifier(perturbed_z), data.test_mask)
    ...
    plt.title('Cora - 混淆矩阵')
    plt.show()
    y_pred_probs = F.softmax(classifier(perturbed_z)[data.test_mask], dim=1).detach().cpu().numpy()
