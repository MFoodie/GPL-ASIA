...
# 定义 Evaluator 类（替换 from evaluator import Evaluator）
class Evaluator:
...
class GAT(nn.Module):
    def __init__(self, nfeat, nclass, nhid, dropout, nheads):
        ...
        self.prompt_graph = None  # placeholder for ProG
...
def stealthy_adversarial_attack(data, model, epsilon=0.25, perturbation_ratio=0.6, task_type='node_classification'):
    ...
    perturbation_ratio = min(perturbation_ratio,
                             0.8 * (1 - original_acc) if task_type == 'node_classification' else 0.6)
    ...
    # 更积极的反馈调整
    max_attempts = 6  # 增加最大尝试次数
    attempt = 0
    perturbed_acc = test_after_attack(perturbed_x, perturbed_edge_index)
    
    # 更严格的成功标准
    target_acc_reduction = 0.20  # 目标准确率降低20%
    while perturbed_acc > original_acc * (1 - target_acc_reduction) and attempt < max_attempts:
        # 更激进的扰动增加
        perturbation_ratio = min(perturbation_ratio * 1.6, 0.9)  # 增加扰动比例上限
        epsilon = min(epsilon * 1.5, 0.5)  # 增加扰动强度上限
        
        # 重新生成更强的攻击
        perturbed_edge_index = add_enhanced_trigger_nodes(data, target_nodes,
                                                         num_trigger_nodes=15 if task_type == 'node_classification' else 10)
        perturbed_edge_index = add_trigger_induced_subgraph(data, perturbation_ratio, task_type)
        perturbed_edge_index = structure_perturbation(data, perturbed_edge_index, perturbation_ratio)
        perturbed_x = enhanced_perturb_node_features(data.x, perturbation_ratio, epsilon, task_type)
...
def structure_perturbation(data, edge_index, perturbation_ratio=0.4):
    ...
    if hasattr(data, 'x') and data.x is not None:
        ...
        edge_importance = edge_importance + feature_similarity * 15  # 特征相似性权重
...
def evaluate_attack_metrics():
    ...
    # 计算攻击效果评估
    original_acc = (original_pred[data.test_mask.to(device)] == data.y.to(device)[data.test_mask.to(device)]).float().mean().item()
    perturbed_acc = (perturbed_pred[data.test_mask.to(device)] == data.y.to(device)[data.test_mask.to(device)]).float().mean().item()
    accuracy_drop = original_acc - perturbed_acc
    print(f'Original Accuracy: {original_acc:.4f}')
    print(f'Perturbed Accuracy: {perturbed_acc:.4f}')
    print(f'Accuracy Drop: {accuracy_drop:.4f}')
    print(f'Attack Effectiveness: {accuracy_drop/original_acc*100:.2f}%')
