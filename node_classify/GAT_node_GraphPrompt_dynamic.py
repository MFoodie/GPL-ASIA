...
class GAT(nn.Module):
    ...
    def set_prompt_graph(self, prompt_graph=None):
        if prompt_graph is None:
            ...
            prompt_graph += tc.randn_like(prompt_graph) * 0.1  # 添加少量噪声
...
def train_with_contrastive_loss_simple():
    ...
    output1 = model(x1 + (model.prompt_graph if model.prompt_graph is not None else 0), e1)
    output2 = model(x2 + (model.prompt_graph if model.prompt_graph is not None else 0), e2)
...
def stealthy_adversarial_attack(data, model, epsilon=0.25, perturbation_ratio=0.5, task_type='node_classification'):
    """
    增强自适应对抗攻击，优化触发图和嵌入参数，显著提升攻击成功率。
    参数：
        - data: 图数据对象
        - model: GAT 模型
        - epsilon: 特征扰动强度（增加到 0.25）
        - perturbation_ratio: 初始扰动比例（增加到 0.5）
        - task_type: 任务类型
    """
    # 使用自适应攻击策略
    ...
    perturbation_ratio = min(perturbation_ratio,
                             0.7 * (1 - original_acc) if task_type == 'node_classification' else 0.5)

    # 选择多个目标节点，增加攻击覆盖面
    target_nodes = select_multiple_target_nodes(data, task_type, num_targets=3)
    
    # 增强触发器设计
    perturbed_edge_index = add_enhanced_trigger_nodes(data, target_nodes,
                                                     num_trigger_nodes=8 if task_type == 'node_classification' else 5)
    ...
    # 更严格的成功标准
    target_acc_reduction = 0.15  # 目标准确率降低15%
    while perturbed_acc > original_acc * (1 - target_acc_reduction) and attempt < max_attempts:
        # 更激进的扰动增加
        perturbation_ratio = min(perturbation_ratio * 1.5, 0.8)  # 增加扰动比例上限
        epsilon = min(epsilon * 1.4, 0.4)  # 增加扰动强度上限
        
        # 重新生成更强的攻击
        perturbed_edge_index = add_enhanced_trigger_nodes(data, target_nodes,
                                                         num_trigger_nodes=10 if task_type == 'node_classification' else 7)
        perturbed_edge_index = add_trigger_induced_subgraph(data, perturbation_ratio, task_type)
        perturbed_edge_index = structure_perturbation(data, perturbed_edge_index, perturbation_ratio)
        perturbed_x = enhanced_perturb_node_features(data.x, perturbation_ratio, epsilon, task_type)
...
# 攻击策略自适应调整
def adaptive_attack_strategy(data, model, base_epsilon=0.25, base_perturbation_ratio=0.5):
    """
    根据模型性能自适应调整攻击策略。
    """
    original_acc = test()
    ...
    if original_acc > 0.8:
        ...
        perturbation_ratio = base_perturbation_ratio * 1.3
        num_attempts = 6
    elif original_acc > 0.6:
        # 中等准确率模型
        epsilon = base_epsilon * 1.2
        perturbation_ratio = base_perturbation_ratio * 1.1
        num_attempts = 5
    else:
        # 低准确率模型使用基础参数
        epsilon = base_epsilon
        perturbation_ratio = base_perturbation_ratio
        num_attempts = 4
    
    return epsilon, perturbation_ratio, num_attempts
...
def select_low_centrality_node(data, task_type='node_classification'):
    ...
    degrees = degree(data.edge_index[0], num_nodes=data.num_nodes)
    if task_type == 'node_classification':
        # 选择低度数节点，但优先选择测试集中的节点
        test_nodes = tc.where(data.test_mask.to(device))[0]
        if len(test_nodes) > 0:
            test_degrees = degrees[test_nodes]
            low_degree_nodes = test_nodes[tc.argsort(test_degrees)[:10]]  # 测试集中的低度数节点
        else:
...
# 增强触发节点攻击
def add_enhanced_trigger_nodes(data, target_nodes, num_trigger_nodes=8):
    """
    添加增强触发节点，针对多个目标节点增加连接以增强攻击效果。
    参数：
        - data: 图数据对象
        - target_nodes: 目标节点列表
        - num_trigger_nodes: 触发节点数量
    """
...
def add_trigger_nodes(data, target_node, num_trigger_nodes=5):
    ...
    参数：
        - data: 图数据对象
        - target_node: 目标节点
        - num_trigger_nodes: 触发节点数量（默认增加到 5）
    """
    num_nodes = data.x.size(0)
    degrees = degree(data.edge_index[0], num_nodes=num_nodes)
...
def add_trigger_induced_subgraph(data, perturbation_ratio=0.2, task_type='node_classification'):
    ...
    num_edges = 3 if task_type == 'node_classification' else 2  # 增加边数量
...
def structure_perturbation(data, edge_index, perturbation_ratio=0.3):
    """
    增强结构扰动，使用更激进的策略删除和添加边。
    参数：
        - data: 图数据对象
        - edge_index: 当前边索引
        - perturbation_ratio: 扰动比例（增加到 0.3）
    """
    ...
    if hasattr(data, 'x') and data.x is not None:
        ...
        edge_importance = edge_importance + feature_similarity * 10  # 特征相似性权重
    ...
    low_degree_nodes = tc.argsort(degrees)[:int(data.num_nodes * 0.4)].cpu().numpy()
...
def enhanced_perturb_node_features(x, perturbation_ratio=0.3, epsilon=0.25, task_type='node_classification', attack_strategy='adaptive'):
    ...
    参数：
        ...
        - epsilon: 扰动强度
        - task_type: 任务类型
        - attack_strategy: 攻击策略 ('adaptive', 'random', 'degree_based')
    """
    perturbed_x = x.clone()
    ...
    if mean_diff > 0.8 or var_diff > 0.8:
...
def perturb_node_features(x, perturbation_ratio=0.2, epsilon=0.15, task_type='node_classification'):
    """
    特征扰动，基于梯度和特征重要性，增强攻击效果。
    参数：
        - x: 原始节点特征
        - perturbation_ratio: 扰动比例（默认增加到 0.2）
        - epsilon: 扰动强度（默认增加到 0.15）
        - task_type: 任务类型
    """
    ...
    top_k_features = int(x.size(1) * perturbation_ratio * 1.5)  # 增加扰动特征数量
    important_features = tc.argsort(feature_importance)[-top_k_features:]
    ...
    adaptive_epsilon = epsilon * feature_variance / (feature_variance.mean() + 1e-8)
    if task_type == 'link_prediction':
        adaptive_epsilon *= 0.7  # 链接预测任务稍减弱扰动
...
def evaluate_with_metrics(perturbed_x, perturbed_edge_index):
    ...
    # 获取正样本和负样本的预测概率
    y_pred_pos = F.softmax(output[data.test_mask.to(device)], dim=1).cpu().detach().numpy()  # 形状 (1708, num_classes)
    y_pred_neg = F.softmax(output[~data.test_mask.to(device)], dim=1).cpu().detach().numpy()  # 形状 (num_nodes - 1708, num_classes)

    # 展平正样本和负样本的最大概率
    y_pred_pos_max = y_pred_pos.max(axis=1)  # 每个节点的最高概率，形状 (1708,)
    y_pred_neg_max = y_pred_neg.max(axis=1)  # 每个节点的最高概率，形状 (num_nodes - 1708,)
...
def calculate_enhanced_asr(original_pred, perturbed_pred, test_mask, y_true, perturbed_x=None, perturbed_edge_index=None):
    """
    增强攻击成功率计算，提供更详细的统计信息。
    """
    # 基本ASR计算 - 修复：比较原始预测和攻击后预测
    successful_attack = (perturbed_pred[test_mask] != original_pred[test_mask]).sum().item()
    total_attack = test_mask.sum().item()
    asr = successful_attack / total_attack
    
    # 计算置信度下降（需要重新获取logits而不是argmax结果）
    model.eval()
    with tc.no_grad():
        ...
        if perturbed_x is not None and perturbed_edge_index is not None:
            ...
            # 如果没有提供扰动数据，使用简化的置信度计算
            perturbed_logits = model(data.x, data.edge_index)
        
        original_conf = F.softmax(original_logits[test_mask], dim=1).max(dim=1)[0].mean().item()
        perturbed_conf = F.softmax(perturbed_logits[test_mask], dim=1).max(dim=1)[0].mean().item()
        confidence_drop = original_conf - perturbed_conf
    
    # 计算类别级别的攻击成功率
    class_asr = {}
    for class_id in range(y_true.max().item() + 1):
        class_mask = (y_true[test_mask] == class_id)
        if class_mask.sum() > 0:
            class_successful = (perturbed_pred[test_mask][class_mask] != original_pred[test_mask][class_mask]).sum().item()
